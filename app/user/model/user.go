...
type UserInfo struct {
	ID        int64          `gorm:"column:id;primaryKey;comment:用户ID"`
	UUID      string         `gorm:"column:uuid;uniqueIndex;type:char(20);comment:用户UUID"`
	Nickname  string         `gorm:"column:nickname;type:varchar(20);comment:用户昵称"`
	Telephone string         `gorm:"column:telephone;index;not null;type:char(11);comment:手机号码"`
	...
	Password  string         `gorm:"column:password;type:char(18);comment:密码"`
	...
	IsAdmin   int8           `gorm:"column:is_admin;not null;comment:是否管理员，0:否，1:是"`
	Status    int8           `gorm:"column:status;not null;comment:用户状态，0:禁用，1:正常"`
}
